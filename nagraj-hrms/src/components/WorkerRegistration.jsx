...
const WorkerRegistration = ({ onClose, onSuccess, session }) => {
  ...
  const checkDuplicateAadhar = async (aadharNumber) => {
    ...
    try {
      ...
      // Check if any worker has same aadhar number
      const isDuplicate = workers.some(w => w.aadhar === aadharNumber);
      if (isDuplicate) {
        showToast(`❌ Aadhar number ${aadharNumber} is already registered!`, true);
        return true;
      }
      return false;
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ DUPLICATE AADHAR CHECK - MAIN VALIDATION
